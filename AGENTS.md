...
这是“地下城：迷踪”的源码仓库：移动端优先、离线运行的单层迷宫探索游戏。保持原生 HTML/CSS/Canvas 2D 与经典 JavaScript，不为普通修复引入框架、后端或构建依赖。

- `index.html`：界面结构和本地资源入口。
- `styles.css`：移动端布局、安全区、弹层与事件时间流。
- `game.js`：地图生成、游戏状态、战斗、剧情、渲染、寻路和本地存档。
- `assets/sprites-atlas.png`、`assets/sprites-atlas.json`：PNG 图集及索引；图标和素材说明也在 `assets/`。
- `tests/smoke.test.js`：原生 Node.js 状态机与 DOM/Canvas 模拟回归。
...
- 所有运行资源使用包内相对路径。不要引入外部 CDN、联网请求、模块脚本、内联脚本、行内事件或动态代码执行。
...
在仓库根目录运行 `npm test`，或直接运行 `node tests/smoke.test.js`；无需安装 npm 依赖。随机地图或事件相关修改可重复运行并保留失败信息，不能把偶现失败当作通过。
